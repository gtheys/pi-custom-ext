 ...
 *
 * Provides an `ask_user_question` tool that pauses execution and asks the
 * user a single question — free-form text, single-select, or multi-select —
 * via an interactive TUI dialog. Users can always pick "Other" to type a
 * custom answer when options are provided.
 */
...
// AIDEV-NOTE: index is required (0 for Other/Submit placeholders) so option
// answers never need non-null assertions.
...
// AIDEV-NOTE: pure helpers exported for index.test.ts only.
...
export function getOtherLabel(options: AskOption[]): string {
  if (options.some((option) => option.label.toLowerCase() === 'other')) {
    return 'Other (custom)'
  }
  return 'Other'
}
...
function buildResult(
  ...
  if (mode === 'text') {
    ...
    if (answer.label.trim().length > 0) {
      text = `User answered: ${answer.label}`
    } else {
      text = 'User submitted an empty response'
    }
...
async function askSingleChoice(
  ...
  return ctx.ui.custom<AskAnswer | null>((tui, theme, _kb, done) => {
    ...
    function render(width: number): string[] {
      // AIDEV-NOTE: The cache MUST be keyed on width: pi-tui calls requestRender()
      // but NOT invalidate() on terminal resize, so render() can be re-entered
      // with a new width. Returning stale wider lines trips the TUI width guard
      ...
      for (let i = 0; i < allOptions.length; i++) {
        ...
        let label = `${option.index}. ${option.label}`
        if (option.isOther) {
          label = option.label
        }
        let styled = theme.fg('text', label)
        if (selected) {
          styled = theme.fg('accent', label)
        }
        add(`${prefix}${styled}`)
...
async function askMultiChoice(
  ...
  return ctx.ui.custom<AskAnswer[] | null>((tui, theme, _kb, done) => {
    ...
    function render(width: number): string[] {
      // See askSingleChoice: cache must be keyed on width or resize crashes the TUI.
      if (cachedLines && cachedWidth === width) return cachedLines
      ...
      for (let i = 0; i < allItems.length; i++) {
        ...
        if (item.isSubmit) {
          ...
          let color: 'success' | 'dim' = 'dim'
          if (selected.size > 0) {
            label = `✓ ${item.label} (${selected.size} selected)`
            color = 'success'
          }
          let styled = theme.fg(color, label)
          if (isFocused) {
            styled = theme.fg('accent', label)
          }
          add(`${prefix}${styled}`)
        ...
        if (item.isOther) {
          ...
          if (isFocused) {
            styled = theme.fg('accent', `${marker} ${item.label}${suffix}`)
          }
          add(`${prefix}${styled}`)
          continue
        }
        ...
        let styled = theme.fg(checked ? 'success' : 'text', label)
        if (isFocused) {
          styled = theme.fg('accent', label)
        }
        add(`${prefix}${styled}`)
...
// mutex lives on globalThis so separate extension files share it without
...
type GlobalWithUiLock = typeof globalThis & {
  __piSharedUiLock?: SharedUiLock
}

function createUiLock(): SharedUiLock {
...
function getSharedUiLock(): SharedUiLock {
  ...
  if (!g[SHARED_UI_LOCK_KEY]) {
    g[SHARED_UI_LOCK_KEY] = createUiLock()
  }
...
export default function askUserQuestion(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      let mode: AskUserQuestionMode = 'text'
      if (options.length > 0) {
      ...
      return withUILock(async () => {
        if (mode === 'text') {
          let editorTitle = params.question
          if (context) {
            editorTitle = `${params.question}\n\n${context}`
          }
    ...
    renderResult(result, _options, theme) {
      ...
      const lines = details.answers.map((answer) => {
        if (answer.type === 'text') {
          const label = answer.label || '(empty response)'
