...
export function muxSetupHint(): string {
  ...
  if (pref === "herdr") {
    return "Start pi inside Herdr (`herdr pi`).";
  }
  return "Start pi inside cmux (`cmux pi`), tmux (`tmux new -A -s pi 'pi'`), zellij (`zellij --session pi`, then run `pi`), WezTerm, or Herdr (`herdr pi`).";
}
 ...
 *
 * Returns an identifier (`surface:42` in cmux, `%12` in tmux, `pane:7` in zellij, `42` in wezterm, `wX:pY` in herdr).
 */
export function createSurface(name: string): string {
  ...
  if (backend === "herdr") {
    return createHerdrSurface(name, process.env.HERDR_PANE_ID);
  }
...
// AIDEV-NOTE: Herdr backend uses the `herdr` JSON CLI. Pane IDs look like `wX:pY`,
// splits default to `right` with `--no-focus` so the parent keeps focus, and reads
// use `--source visible` because `recent` only captures the focused pane.
// ── Herdr helpers ──

function parseHerdrJson<T>(output: string): T | null {
  try {
...
function parseHerdrPaneSurface(output: string, context: string): string {
  const parsed = parseHerdrJson<{ result?: { pane?: { pane_id?: string } } }>(output);
  const paneId = parsed?.result?.pane?.pane_id;
  if (!paneId) {
    throw new Error(`Unexpected herdr ${context} output: ${output}`);
  }
  return paneId;
}

function createHerdrSurface(name: string, fromSurface?: string): string {
  const args = ["pane", "split", "--direction", "right", "--no-focus", "--cwd", process.cwd()];
  if (fromSurface) {
  ...
  const surface = parseHerdrPaneSurface(output, "split");

  try {
    execFileSync("herdr", ["pane", "rename", surface, name], { encoding: "utf8" });
  } catch {
    // Pane rename is cosmetic; don't fail the launch if it doesn't work.
  }
...
export function createSurfaceSplit(
  ...
  if (backend === "herdr") {
    // Herdr's split command only supports right and down. Normalize left/up
    // to the closest supported direction; createSurface never requests those.
    const herdrDirection = direction === "up" ? "down" : "right";
    const args = ["pane", "split", "--direction", herdrDirection, "--no-focus", "--cwd", process.cwd()];
    if (fromSurface) {
      args.push("--pane", fromSurface);
    } else {
      args.push("--current");
...
export function sendCommand(surface: string, command: string): void {
  ...
  if (backend === "herdr") {
    ...
    execFileSync("herdr", ["pane", "send-keys", surface, "return"], { encoding: "utf8" });
    return;
  }
...
export function readScreen(surface: string, lines = 50): string {
  ...
  if (backend === "herdr") {
    const raw = execFileSync(
      "herdr",
      ["pane", "read", surface, "--source", "visible", "--lines", String(lines)],
      { encoding: "utf8" },
    );
    return tailLines(raw, lines);
  }
...
export function closeSurface(surface: string): void {
  ...
  if (backend === "herdr") {
    try {
      ...
      // Already gone or not a herdr pane.
    }
    return;
  }
